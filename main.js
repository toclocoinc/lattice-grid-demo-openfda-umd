 ...
 *
 *   (nothing)            live, reading the openFDA drug-event API and polling
...
(function (root) {
  ...
  const TITLE = 'Adverse drug reactions reported to the FDA, live';
  ...
  function showError(error) {
    ...
    panel.className = 'loading';
    const title = document.createElement('h1');
    title.textContent = 'The adverse drug reaction data could not be loaded';
    const message = document.createElement('p');
    message.className = 'loading-message';
    ...
    host.append(panel);
    console.error('[openfda demo]', error);
  }
  ...
  async function start() {
    ...
    try {
      ...
      const { buildDashboard, fetchInitial, readSnapshot, startPolling, POLL_MS } = root.OpenFdaDemo;
      ...
      if (mode === 'snapshot') {
        ...
        sex = saved.sex;
        serious = saved.serious;
        drugs = saved.drugs;
        meta = saved.meta;
        update('Building the dashboard...', 1);
      } else {
        const update = showProgress('Reading the openFDA drug adverse-event API...');
        try {
          const initial = await fetchInitial({ onProgress: update });
          terms = initial.terms;
          reports = initial.reports;
          sex = initial.sex;
          serious = initial.serious;
          drugs = initial.drugs;
          meta = { live: true, fetchedAt: Date.now() };
        } catch (liveError) {
          console.warn('[openfda demo] the live fetch failed, falling back to the saved copy:', liveError);
          update('The openFDA API could not be reached. Opening the saved copy...', 1);
          const saved = await readSnapshot();
          terms = saved.terms;
          reports = saved.reports;
          sex = saved.sex;
      ...
      const timings = {
        ...
        terms: built.termsGrid ? built.termsGrid.rows.count() : 0,
      ...
      root.__openFdaDemo = Object.assign(built, { meta, timings, ready: true });
      console.log('[openfda demo] ready', timings);
    } catch (error) {
      root.__openFdaDemo = { ready: false, error: String((error && error.message) || error) };
      showError(error);
    }
  }
